...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    | formal ',' formal_list { $$ = append_Formals(single_Formals($1), $3); }
    ;
    ...
    | expr ',' expr_comma_list { $$ = append_Expressions(single_Expressions($1), $3); }
    ;

    expr_semicolon_list
    : expr_semicolon_list expr';' { $$ = append_Expressions($1, single_Expressions($2)); }
    | expr';' { $$ = single_Expressions($1); }
    ...
    | expr'.'OBJECTID'(' expr_comma_list ')' { $$ = dispatch($1, $3, $5); }
    | expr'@'TYPEID'.'OBJECTID'(' ')' { $$ = static_dispatch($1, $3, $5, nil_Expressions()); }
    | expr'@'TYPEID'.'OBJECTID'(' expr_comma_list ')' { $$ = static_dispatch($1, $3, $5, $7); }
    | OBJECTID'(' ')' { $$ = dispatch(object(stringtable.add_string("self")), $1, nil_Expressions()); }
    | OBJECTID'(' expr_comma_list ')' { $$ = dispatch(object(stringtable.add_string("self")), $1, $3); }
    | IF expr THEN expr ELSE expr FI { $$ = cond($2, $4, $6); }
    | WHILE expr LOOP expr POOL { $$ = loop($2, $4); }
    | '{' expr';' expr_semicolon_list '}' { $$ = block(append_Expressions(single_Expressions($2), $4)); }
    | LET let_list { $$ = $2; }
